...
[filename, pathname] = uigetfile('*.csv', 'Select the high rate gyroscope file (sensor_gyro_fifo_0');
...
rawImuData_highrate = rawImuData_highrate';

%% Get low rate data
% Get sensor accel file
[filename, pathname] = uigetfile('*.csv', 'Select the file to use for temperature - sensor_accel_0');
if isequal(filename,0)
    disp('User canceled file selection.');
    return;
end
...
%get rid of nans
idx_Tclean = ~isnan(imuT);
px4LowRateData_clean = imuData(:, idx_Tclean);
imuT= imuT(idx_Tclean);

%then of outliers
idx_Tclean = ~isoutlier(imuT, 'mean');
px4LowRateData_clean = px4LowRateData_clean(:, idx_Tclean);
...
idx_clean = ~isoutlier(px4HighRateData(2:4,:)', 'mean','ThresholdFactor', 5)';
...
ylabel(tileLayoutObj,'$\mathbf{measurement (rad/s)}$', 'fontweight', 'bold', 'Interpreter', 'latex');
